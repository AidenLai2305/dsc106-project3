...
d3.select("#select-all").on("change", function () {
  ...
  d3.selectAll('#mouse-controls input[type=checkbox]:not(#select-all)')
    .property("checked", checked);

  // Re-trigger path visibility update
  const selected = new Set(
    d3.selectAll("#mouse-controls input:checked:not(#select-all)")
      .nodes()
      .map(n => n.value)
  );
...
document.getElementById("run-button").addEventListener("click", () => {
  ...
  const checkedInputs = Array.from(
    document.querySelectorAll('#mouse-controls input[type="checkbox"]:not(#select-all):checked')
  );
  ...
  runner
  .style("display", "block")
  .attr("fill", colorScale(selectedID)); // match color to selected mouse
